...
@todo put into class, move js if possible
*/
function cleverness_todo_dashboard_widget() {
	...
	// get to-do items
	$results = CTDL_Lib::get_todos($user, $limit, 0, $cat_id);
	...
	if ($results) {
		...
		foreach ($results as $result) {
			...
			if ( CTDL_Loader::$settings['show_progress'] == '1' && $result->progress != '' )
				echo ' <small>['.$result->progress.'%]</small>';
			if ( CTDL_Loader::$settings['list_view'] == '1' && $cleverness_todo_dashboard_settings['dashboard_author'] == '0' ) {
				if ($result->author != '0') {
