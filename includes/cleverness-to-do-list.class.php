...
class ClevernessToDoList {
	...
	protected function show_table_headings( $completed = 0 ) {
		...
		if ( $this->settings['show_id'] ) $this->list .= '<th>'.__( 'ID', 'cleverness-to-do-list' ).'</th>';
	 ...
	 */
	protected function show_id( $todo_field_data ) {
		if ( $this->settings['show_id'] == 1 ) {
			$this->list .= ( $todo_field_data->id != '' ? sprintf( '<td>%s</td>', $todo_field_data->id ) : '<td></td>' );
		}
	}
